...
const ProjectCard = ({
  ...
  return (
    ...
    >
      <div className="flex flex-col overflow-hidden rounded-[28px] bg-[#E9E3D7] text-[#0E1016] shadow-2xl sm:rounded-[28px] md:flex-row md:items-stretch md:rounded-[40px]">
        {/* ── Text ── */}
        <div className="flex flex-1 flex-col justify-center p-7 sm:p-9 md:p-12 lg:p-16 [@media(max-height:600px)]:p-6">
          ...
          {/* meta */}
          <p className="mt-5 text-[11px] font-medium uppercase tracking-[0.16em] text-[#0E1016]/65 sm:mt-7 sm:text-[12px] md:mt-9 md:text-[13px]">
            {technologies.join(" • ")}
          </p>

          {/* title */}
          <h3
            className={`${monaSans.className} mt-2 text-[32px] font-black uppercase leading-[0.95] tracking-[-0.01em] sm:mt-2 sm:text-[44px] sm:leading-[0.92] md:text-[58px] lg:text-[68px] [@media(max-height:600px)]:text-[36px]`}
          >
            {name}
          </h3>

          {/* description */}
          <p className="mt-5 max-w-[440px] text-[14px] leading-relaxed text-[#0E1016]/75 sm:mt-4 sm:text-[15px] md:text-[16px]">
            {description}
          </p>
        ...
        <div className="relative order-first m-4 aspect-[16/10] overflow-hidden rounded-[20px] bg-[#0E1016] sm:m-4 sm:rounded-[20px] md:order-none md:m-5 md:aspect-auto md:h-auto md:w-[46%] md:rounded-[28px]">
          <Image
            src={image}
            alt={name}
            fill
